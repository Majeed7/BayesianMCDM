...
from .StandardAHP import StandardBWM
import numpy as np
...
class GaussianBWM(StandardBWM, MCDMProblem):
    ...
    _basicModel = """
        ...
        parameters {             
            ...
            vector<lower=0>[CNo] mu_AW[DmNo];
            vector<lower=0>[CNo] sigma_AW[DmNo];

            vector<lower=0>[CNo] AB[DmNo];
            vector<lower=0>[CNo] mu_AB[DmNo];
            vector<lower=0>[CNo] sigma_AB[DmNo];
        } 
        ...
        model {
            ...
            for (i in 1:DmNo){
                ...
                AB[i] ~ normal(AB_md[i], sigma_AB[i]);
                AW[i] ~ normal(AW_md[i], sigma_AW[i]);
            }
